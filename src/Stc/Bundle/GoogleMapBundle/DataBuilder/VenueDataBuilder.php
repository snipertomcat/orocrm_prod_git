...
// @todo change to VenueMapBuilder
class VenueDataBuilder extends AbstractDataBuilder
{
    protected $manager;
    ...
    {
        $repository = $this->manager->getRepository('StcVenueBundle:Venue');
        $coordinates = $repository->getAllVenuesWithCoordinates();
    ...
    /**
     * The idea of this method is to return a map that includes all venues regardless of geolocation status.
    ...
    {
        $repository = $this->manager->getRepository('StcVenueBundle:Venue');
        $coordinates = $repository->getAllVenuesWithLookupFields();
        ...
        foreach ($coordinates as $coordinate) {
            if ($coordinate['jjwgMapsLatC'] !== '') {
                ...
                if ($coordinate['address'] !== '' && $coordinate['address'] !== null) {
                    ...
                    $venueObject = $repository->find($coordinate['id']);
                    $venueObject->setJjwgMapsLatC($lat);
                    $venueObject->setJjwgMapsLngC($lng);
                    $this->manager->persist($venueObject);

                    $this->addMarker($venueObject);
                }
            }
        }
        $this->manager->flush();
        ...
        /*
         * use this statement to verify the number of venues against the database:
         */
